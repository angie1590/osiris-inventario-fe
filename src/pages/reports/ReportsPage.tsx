...
function MovementReport({
  ...
  const handleExport = async (fmt: "pdf" | "excel") => {
    ...
    try {
      ...
      downloadBlob(
        res,
        `${prefix}_${range.date_from}_${range.date_to}.${fmt === "pdf" ? "pdf" : "xlsx"}`,
      );
    } catch {
      ...
      <div className="flex flex-wrap items-end gap-3 rounded-lg border bg-card p-3">
        ...
        {supportsProductFilter && (
          <div className="space-y-1">
            ...
            <Select
              value={productId ? String(productId) : "__all__"}
              onValueChange={(v) =>
                setProductId(v === "__all__" ? undefined : Number(v))
              }
            >
...
function MovimientosPorUsuarioReport() {
  ...
  const handleExport = async (fmt: "pdf" | "excel") => {
    ...
    try {
      ...
      downloadBlob(
        res,
        `movimientos_usuario_${range.date_from}_${range.date_to}.${fmt === "pdf" ? "pdf" : "xlsx"}`,
      );
    } catch {
      toast({ variant: "destructive", description: "Error al exportar" });
    }
