...
export default function BajasPage() {
  ...
  const { data: docs, isLoading } = useBajas({
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">Bajas de Inventario</h1>
        {canCreate && (
          <Button onClick={() => navigate('/inventory/bajas/new')}>
            <Plus className="mr-2 h-4 w-4" />Nueva baja
          </Button>
        )}
      </div>
                    ...
                    <TableRow key={d.id}>
                      ...
                      <TableCell>
                        <Button variant="ghost" size="sm" asChild>
                          <Link to={`/inventory/bajas/${d.id}`}>Ver</Link>
                        </Button>
                      </TableCell>
                    </TableRow>
                  ))}
