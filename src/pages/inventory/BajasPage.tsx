...
const columns: Column<InventoryDocument>[] = [
  ...
  { key: 'notes', header: 'Notas', cell: (d) => d.notes || '—' },
  ...
  { key: 'actions', header: '', cell: (d) => <Button variant="ghost" size="sm" asChild><Link to={`/inventory/bajas/${d.id}`}>Ver</Link></Button> },
]
...
export default function BajasPage() {
  ...
  const { data: docs, isLoading, isError, refetch } = useBajas({ date_from: dateFrom || undefined, date_to: dateTo || undefined, cursor })

  return (
    <div className="space-y-4">
      <PageHeader
        title="Bajas de Inventario"
        actions={canCreate && <Button onClick={() => navigate('/inventory/bajas/new')}><Plus className="mr-2 h-4 w-4" />Nueva baja</Button>}
      />
      ...
      <DataTable columns={columns} data={docs ?? []} rowKey={(d) => d.id} isLoading={isLoading} isError={isError} onRetry={refetch} emptyHeading="Sin bajas de inventario" />
