...
import { usePendingRecategorization, useRecategorize, useCategories } from '@/features/catalog/hooks'
...
export default function RecategorizePage() {
  ...
  const groups = useMemo(() => {
    ...
    return Array.from(byDefault.entries()).map(([defaultId, products]) => {
      ...
      const targets = parentId != null ? subtreeOf(cats, parentId).filter((c) => !c.is_default) : []
      return { defaultId, parentId, parentPath: parentId != null ? buildCategoryPath(cats, parentId) : '—', products, targets }
    })
  }, [pending, cats])

  const selectedCount = Object.values(assignments).filter((v) => v != null).length

  const handleSubmit = async () => {
    const payload = Object.entries(assignments)
      .filter(([, catId]) => catId != null)
      .map(([pid, catId]) => ({ product_id: Number(pid), category_id: catId as number }))
    if (payload.length === 0) {
      toast({ variant: 'warning', title: 'Nada que guardar', description: 'Asigna una categoría a al menos un producto.' })
      return
    }
    try {
      const res = await recategorize.mutateAsync(payload)
      toast({ variant: 'success', title: 'Productos recategorizados', description: `${res.recategorized} producto(s) reasignado(s).` })
      setAssignments({})
    } catch (err: unknown) {
      toast({ variant: 'destructive', title: 'Error al recategorizar', description: getApiErrorMessage(err, 'No se pudo recategorizar. Intenta nuevamente.') })
    }
  }
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        actions={
          (pending ?? []).length > 0 && (
            <Button onClick={handleSubmit} isLoading={recategorize.isPending} disabled={selectedCount === 0}>
      ...
      {(pending ?? []).length === 0 ? (
        <EmptyState
          ...
          action={{ label: 'Ir a productos', onClick: () => navigate('/products') }}
        />
      ) : (
        <>
          <div className="flex items-start gap-2 rounded-lg border border-amber-400/80 bg-amber-100/95 px-4 py-2.5 text-sm text-amber-900">
            ...
            <span>
              Estos productos quedaron en una categoría temporal <strong>"Sin clasificar"</strong> al
              crear subcategorías. Asígnales una subcategoría definitiva. La categoría temporal se
          ...
          {groups.map((g) => (
            <div key={g.defaultId} className="rounded-lg border bg-card">
              <div className="border-b px-4 py-2 text-sm font-medium text-muted-foreground">
                {g.parentPath} <span className="text-foreground">/ Sin clasificar</span> — {g.products.length} producto(s)
              </div>
                ...
                {g.products.map((p) => (
                  <div key={p.id} className="flex items-center gap-4 px-4 py-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">ISBN {p.isbn}</p>
                    </div>
                    <div className="w-72">
                      <TreeSelector
                        categories={g.targets}
                        value={assignments[p.id] ?? null}
                        onChange={(id) => setAssignments((prev) => ({ ...prev, [p.id]: id }))}
