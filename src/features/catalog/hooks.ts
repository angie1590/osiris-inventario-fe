...
export function useCategoryAttributes(id: number) {
  return useQuery({
    ...
    queryFn: () =>
      api
        .get<CategoryAttribute[]>(`/categories/${id}/attributes`)
        .then((r) => r.data),
    enabled: !!id,
  });
}
...
export function useCreateCategory() {
  ...
  return useMutation({
    mutationFn: (payload: CreateCategoryPayload) =>
      api
        .post<Category & { products_moved?: number }>("/categories", payload)
        .then((r) => r.data),
    onSuccess: () => {
...
export function useDeleteCategory() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      api.delete(
        `/categories/${id}`,
        deleteProducts ? { params: { delete_products: true } } : undefined,
      ),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["categories"] });
      qc.invalidateQueries({ queryKey: ["products"] });
...
export function useUpdateAttribute() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      api
        .patch<
          CategoryAttribute & { remap_pending?: number }
        >(`/categories/${categoryId}/attributes/${attrId}`, payload)
        .then((r) => r.data),
    onSuccess: (_d, v) => {
      qc.invalidateQueries({ queryKey: ["category-attributes", v.categoryId] });
...
export function useDeactivateAttribute() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      api
        .post<CategoryAttribute>(
          `/categories/${categoryId}/attributes/${attrId}/deactivate`,
        )
        .then((r) => r.data),
    onSuccess: (_d, v) =>
      qc.invalidateQueries({ queryKey: ["category-attributes", v.categoryId] }),
...
export function useReactivateAttribute() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      api
        .post<CategoryAttribute>(
          `/categories/${categoryId}/attributes/${attrId}/reactivate`,
        )
        .then((r) => r.data),
    onSuccess: (_d, v) =>
      qc.invalidateQueries({ queryKey: ["category-attributes", v.categoryId] }),
...
export function usePendingRecategorization() {
  return useQuery({
    ...
    queryFn: () =>
      api
        .get<Product[]>("/products/pending-recategorization")
        .then((r) => r.data),
    staleTime: 60 * 1000,
  });
}
  ...
  return useMutation({
    mutationFn: ({
      ...
      api
        .patch<Product>(
          `/products/${id}/status`,
          categoryId != null ? { status, category_id: categoryId } : { status },
        )
        .then((r) => r.data),
    onSuccess: (_d, v) => {
      qc.invalidateQueries({ queryKey: ["products"] });
