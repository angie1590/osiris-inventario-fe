...
import { useCreateProduct, useUpdateProduct, useCategories, useCategoryAttributes } from '@/features/catalog/hooks'
import { useStockMode, formatQuantity, useInternalCodeEnabled, useIsbnRequired } from '@/hooks/useStockMode'
...
const schema = z.object({
  // ISBN length/required validated in onSubmit (depends on the isbn_required param).
...
function CatalogAttributeField({ attr, value, onChange, error }: { attr: CategoryAttribute; value: unknown; onChange: (v: unknown) => void; error?: string }) {
...
function AttributeField({ attr, value, onChange, error }: { attr: CategoryAttribute; value: unknown; onChange: (v: unknown) => void; error?: string }) {
  switch (attr.data_type) {
    case 'catalog':
      return <CatalogAttributeField attr={attr} value={value} onChange={onChange} error={error} />
    case 'boolean':
      return (
      ...
      return (
        <FormField label={attr.name} required={attr.is_required} error={error}>
          <Select value={String(value ?? '')} onValueChange={onChange}>
            <SelectTrigger aria-invalid={!!error}><SelectValue placeholder={`Seleccionar ${attr.name}`} /></SelectTrigger>
            <SelectContent>
      ...
      return (
        <FormField label={attr.name} required={attr.is_required} error={error}>
          <Input
            ...
            min={(attr.data_type === 'integer' || attr.data_type === 'decimal') && !attr.allow_negative ? 0 : undefined}
            aria-invalid={!!error}
            value={String(value ?? '')}
...
export function ProductForm({ product, onSuccess, onCancel, layout }: ProductFormProps) {
  ...
  const isbnRequired = useIsbnRequired()
  ...
  const [orphanPrompt, setOrphanPrompt] = useState<{ targetCategory: number; orphans: string[] } | null>(null)

  const { register, handleSubmit, watch, control, setValue, formState: { errors, isSubmitting }, setError, reset } = useForm<FormData>({
    resolver: zodResolver(schema),
    defaultValues: { stock_minimo: '0' },
  })

  // The category whose attribute values are currently consistent (for revert).
  const confirmedCategoryRef = useRef<number | null>(product?.category_id ?? null)
  ...
  useEffect(() => {
    ...
    if (categoryId == null || categoryId === confirmedCategoryRef.current) return
  ...
  const confirmOrphanCleanup = () => {
    ...
    setCustomAttrs((prev) => {
      const next = { ...prev }
      orphanPrompt.orphans.forEach((k) => { delete next[k] })
      return next
    })
  ...
  const validateCustomAttrs = (): boolean => {
    ...
    for (const a of attrs ?? []) {
      ...
      } else if (!empty && (a.data_type === 'integer' || a.data_type === 'decimal')) {
        const num = Number(v)
        if (Number.isNaN(num)) errs[a.name] = 'Debe ser un número'
        else if (!a.allow_negative && num < 0) errs[a.name] = 'No admite valores negativos'
        else if (a.data_type === 'integer' && !Number.isInteger(num)) errs[a.name] = 'Debe ser un entero'
      }
    }
    setAttrErrors(errs)
  ...
  const onSubmit = async (data: FormData) => {
    ...
    // ISBN: required is parametrizable; length only enforced when provided.
    const isbnVal = (data.isbn ?? '').trim()
    if (isbnRequired && !isbnVal) {
      setError('isbn', { message: 'Requerido' })
      return
    }
    ...
    const payload = {
      ...
      ...(internalCodeEnabled ? { codigo_interno: data.codigo_interno?.trim() || null } : {}),
      name: data.name,
    ...
    try {
      ...
      if (fieldErrors) {
        const knownFields: (keyof FormData)[] = ['isbn', 'name', 'category_id', 'stock_minimo', 'pvp']
        knownFields.forEach((field) => {
          if (fieldErrors[field]) setError(field, { message: fieldErrors[field] })
        })
      }
      if (code === 'INVALID_QUANTITY') setError('stock_minimo', { message: msg })
  ...
  const body = (
    <>
      ...
      <Section title="Información general">
        {internalCodeEnabled && (
          <FormField label="Código interno" error={errors.codigo_interno?.message} hint="Código alfanumérico (ej: L010263).">
            <Input {...register('codigo_interno')} placeholder="L010263" />
          </FormField>
        )}
        <FormField
          label="ISBN"
          required={isbnRequired}
          error={errors.isbn?.message}
          hint={isbnRequired ? undefined : 'Si no lo envías, el backend generará un código automático.'}
        >
          <Input {...register('isbn')} placeholder="9780000000000" aria-invalid={!!errors.isbn} />
        </FormField>
        ...
        <div className="grid grid-cols-2 gap-4">
            ...
            {(() => {
              ...
              return (
                <Input
                  ...
                  {...reg}
                  onChange={(e) => {
                    if (integerMode) e.target.value = e.target.value.replace(/[.,].*$/, '')
                    reg.onChange(e)
                  }}
                />
              )
          ...
          {isEdit && (
            <FormField label="Stock actual" hint="Solo lectura — se actualiza via movimientos">
              <Input value={formatQuantity(product?.stock_actual ?? 0, integerMode)} disabled className="bg-muted" />
            </FormField>
          )}
        </div>
      </Section>
            ...
            return (
              <Input
                ...
                onChange={(e) => {
                  ...
                  if (parts[1]?.length > 2) e.target.value = `${parts[0]}.${parts[1].slice(0, 2)}`
                  reg.onChange(e)
                }}
              />
            )
                ...
                <AttributeField
                  ...
                  onChange={(v) => {
                    ...
                    setAttrErrors((prev) => {
                      ...
                      const next = { ...prev }; delete next[attr.name]; return next
                    })
                  }}
                />
              ))}
  ...
  const footer = (
    <>
      ...
      <Button type="submit" isLoading={isSubmitting}>{isEdit ? 'Actualizar' : 'Crear producto'}</Button>
    </>
  )
  ...
  const orphanDialog = orphanPrompt && (
    <ConfirmDialog
      ...
      description={
        <>
          Al cambiar de categoría, estos atributos ya no aplican y se eliminarán sus
          valores: <strong>{orphanPrompt.orphans.join(', ')}</strong>. ¿Continuar?
        </>
      }
