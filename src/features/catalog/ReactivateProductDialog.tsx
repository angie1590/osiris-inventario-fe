...
export function ReactivateProductDialog({
  ...
  const handleConfirm = async () => {
    ...
    if (categoryId == null) {
      setFormError(
        "Selecciona una categoría activa para reactivar el producto.",
      );
      return;
    }
    setSubmitting(true);
    try {
      ...
      toast({
        ...
        description: `"${product.name}" reactivado.`,
      });
      onClose();
    } catch (err: unknown) {
      setFormError(
      ...
      <DialogContent className="max-w-md">
        ...
        <DialogBody className="space-y-3">
          ...
          <p className="text-sm text-muted-foreground">
            La categoría de <strong>{product.name}</strong> fue eliminada.
            Selecciona una categoría activa para reactivarlo.
          </p>
