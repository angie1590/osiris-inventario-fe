...
export function VoidDialog({ doc, onClose, onVoided }: Props) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      await voidDoc.mutateAsync({
        id: doc.id,
        authorizerPin: needsPin ? normalizedPin : undefined,
      });
      toast({
        variant: "success",
      ...
      setError(
        getApiErrorMessage(err, "No se pudo anular el documento.", {
          VOID_PIN_REQUIRED:
  ...
  return (
    <Dialog
      open
      onOpenChange={(o) => {
        if (!o && !voidDoc.isPending) onClose();
      }}
    >
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Se revertirá su efecto en stock, Kardex y reportes. Esta acción no
            se puede deshacer.
          </DialogDescription>
        </DialogHeader>
